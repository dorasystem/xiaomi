...
{{--        </div>--}}
        <div class="container my-5">
            <div class="row">
                @foreach($histories as $history)
                    <div class="col-md-3 col-sm-6">
                        <h1 style="font-size: 50px" class="border-top border-3 pt-4 text-lightgrey fw-bold mt-3">{{ $history->year }}</h1>
                        <div class="lh-normal text-black">{!! $history['description_' . $lang] !!}</div>
                    </div>
                @endforeach
            </div>
            <div class="row my-5">
                <div class="col-md-6"><img style="object-position: right" class="w-100 rounded h-100 fit-cover" src="{{ asset('storage/' . $about->image) }}" alt="" /></div>
            ...
            <div class="row my-5">
                ...
                <div class="order-md-2 order-1 col-md-6"><img class="w-100 rounded h-100 fit-cover" src="{{ asset('storage/' . $about->photo) }}" alt="" /></div>
            </div>
            <div class="row">
                <div class="row align-items-center justify-content-between">
                    <h1 class="col-md-8">@lang('home.command')</h1>
                    <div class="col-md-4 text-md-end mb-3">
                        <a href="/career" class="border-1 rounded-2 btn-blackborder py-1 px-3">@lang('home.all_vacancy') <img src="/assets/icons/arrow.svg" alt="" /></a>
                    </div>
                </div>
                    ...
                    @foreach($careers as $item)
                        <a href="/career" class="col-md-3 col-sm-6 rounded mb-3">
                            <div class="bg-white rounded">
                                ...
                                <div style="font-size: 11px" class="px-3 small fw-bold text-grey">{{ $item['title_' . $lang] }}</div>

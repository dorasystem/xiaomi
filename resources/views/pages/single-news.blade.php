...
@section('content')
    <main class="container">
        ...
        <div class="row align-items-center justify-content-between mb-lg-0 mb-2">
            <h2 class="order-lg-1 order-2 col-lg-10 fw-normal hover-orange mb-3">{{ $news['title_' . $locale] ?? 'Ожидаемый релиз: когда выйдет флагманская линейка Xiaomi 15'}}</h2>
            <div class="order-lg-2 order-1 col-lg-2 d-flex align-items-center gap-2">
                <img src="/assets/icons/calendar-icon.svg" alt="" />
                <div class="text-grey">{{ $news->date }}</div>
            </div>
        </div>
        <div class="mb-5">
            {!! $news['description_' . $locale] !!}
            <br /><br />
            {!! $news['content_' . $locale] ?? "Какие будут характеристики Xiaomi 15
                ...
                @if(!empty($otherNews) && $otherNews->count())
                    @foreach($otherNews as $item)
                        <div class="swiper-slide product shadow-sm position-relative rounded">
                            <a href="/" class="mb-3">
                                <img height="250px" class="w-100 fit-cover" src="{{ asset('storage/' . $item->image) ?? '/assets/images/news1.jpg'}}" alt="" />

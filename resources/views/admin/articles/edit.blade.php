...
@section('content')
    <form action="{{ route('articles.update', $article->id) }}" method="POST" enctype="multipart/form-data" novalidate class="needs-validation" onsubmit="updateEditorContent()">
        @csrf
        @method('PUT')
                ...
                <div class="page-header">
                    <div class="page-header-left d-flex align-items-center">
                        <div class="page-header-title">
                            <h5 class="m-b-10">Редактировать Полезные статьи </h5>
                        </div>
                        <ul class="breadcrumb">
                                        ...
                                        @foreach (['uz', 'en', 'ru'] as $lang)
                                            <div class="tab-pane fade show {{ $lang == 'uz' ? 'active' : '' }}" id="{{ $lang }}Content">
                                                <div class="form-group pb-3">
                                                    ...
                                                    <input type="text" class="form-control" id="title_{{ $lang }}" name="title_{{ $lang }}" value="{{ old('title_' . $lang, $article->{'title_' . $lang}) }}" required>
                                                </div>
                                                <div class="form-group pb-3">
                                                    <label for="description_{{ $lang }}">Oписание ({{ strtoupper($lang) }}):</label>
                                                    <div id="descriptionEditor_{{ $lang }}" style="height:200px;">{!! old('description_' . $lang, $article->{'description_' . $lang}) !!}</div>
                            ...
                            <div class="card stretch">
                                ...
                                <div class="card-body p-4">
                                    <div class="form-group pb-3">
                                        ...
                                        @if ($article->image)
                                            <img src="{{ asset('storage/' . $article->image) }}" alt="Current Image" class="img-thumbnail mt-2" width="200">
                                        @endif
                                    </div>
                                    <div class="form-group pb-3">
                                        <label for="date">Дата:</label>
                                        <input type="date" class="form-control" id="date" name="date" value="{{ old('date', $article->date ? \Carbon\Carbon::parse($article->date)->format('Y-m-d') : '') }}">
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        editor{{ ucfirst($lang) }}.root.innerHTML = `{!! old('content_' . $lang, $article->{'content_' . $lang}) !!}`;
        descriptionEditor{{ ucfirst($lang) }}.root.innerHTML = `{!! old('description_' . $lang, $article->{'description_' . $lang}) !!}`;
        @endforeach

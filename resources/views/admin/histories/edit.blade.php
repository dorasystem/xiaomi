...
@section('content')
    <form action="{{ route('histories.update', $history->id) }}" method="POST" class="needs-validation" onsubmit="updateEditorContent()">
        @csrf
        @method('PUT')
                ...
                <div class="page-header">
                    <div class="page-header-left d-flex align-items-center">
                        <div class="page-header-title">
                            <h5 class="m-b-10">Редактировать история</h5>
                        </div>
                        <ul class="breadcrumb">
                                ...
                                <div class="card-body p-4">
                                    ...
                                    <div class="tab-content pt-3">
                                        <div class="form-group pb-3">
                                            ...
                                            <input type="text" class="form-control" id="year" name="year" value="{{ old('year', $history->{'year'}) }}" required oninput="this.value = this.value.replace(/[^0-9]/g, '');">
                                        </div>
                                        @foreach (['uz', 'en', 'ru'] as $lang)
                                            <div class="tab-pane fade show {{ $lang == 'uz' ? 'active' : '' }}" id="{{ $lang }}Content">
                                                <div class="form-group pb-3">
                                                    <label for="text_{{ $lang }}">Описание ({{ strtoupper($lang) }}):</label>
                                                    <div id="editor_{{ $lang }}" style="height:200px;">{!! old('description+' . $lang, $history->{'description_' . $lang}) !!}</div>
                                                    <input type="hidden" id="text_{{ $lang }}" name="description_{{ $lang }}" value="{{ old('description_' . $lang, $history->{'description_' . $lang}) }}">
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>

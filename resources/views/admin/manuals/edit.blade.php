...
@section('content')
    <form action="{{ route('manuals.update', $manual->id) }}" method="POST" enctype="multipart/form-data" class="needs-validation">
        @csrf
        @method('PUT')
                ...
                <div class="page-header">
                    <div class="page-header-left d-flex align-items-center">
                        <div class="page-header-title">
                            <h5 class="m-b-10">Редактировать руководство</h5>
                        </div>
                        <ul class="breadcrumb">
                                        ...
                                        @foreach (['uz', 'en', 'ru'] as $lang)
                                            <div class="tab-pane fade show {{ $lang == 'uz' ? 'active' : '' }}" id="{{ $lang }}Content">
                                                <div class="form-group pb-3">
                                                    ...
                                                    <input type="text" class="form-control" id="name_{{ $lang }}" name="name_{{ $lang }}" value="{{ old('name_' . $lang, $manual->{'name_' . $lang} ) }}">
                                                </div>
                                                <div class="form-group pb-3">
                                                    ...
                                                    @if($manual->{'pdf_' . $lang})
                                                        <p><a href="{{ asset('storage/' . $manual->{'pdf_' . $lang}) }}" target="_blank">Текущий файл</a></p>
                                                    @endif
                                                </div>
                                            </div>
                                        @endforeach
